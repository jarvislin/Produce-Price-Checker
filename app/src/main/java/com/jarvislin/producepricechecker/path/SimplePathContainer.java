...
public class SimplePathContainer extends PathContainer {
    ...
    public SimplePathContainer() {
        super(R.id.screen_switcher_tag);
        this.contextFactory = Path.contextFactory();
    }
    ...
    private int getLayout(Path path) {
        ...
        if (layoutResId == null) {
            PathLayout layout = (PathLayout) pathType.getAnnotation(PathLayout.class);
            if (layout == null) {
                throw new IllegalArgumentException(String.format("@%s annotation not found on class %s", PathLayout.class.getSimpleName(), pathType.getName()));
            }
